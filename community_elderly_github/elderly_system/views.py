...
def admin_dashboard(request):
    ...
    # 进行中活动：状态为open/ongoing且未过报名截止时间（open状态）或未结束（ongoing状态）
...
def admin_statistics_export(request):
    ...
    total_abnormal = registrations.filter(status='registered').count()  # 报名未签到
